...
describe('FindNotificationByParticipationRequestUseCase', () => {
  let findNotificationsByParticipationRequestUseCase : FindNotificationsByParticipationRequestUseCase;
  ...
  beforeEach(() => {
    ...
    findNotificationsByParticipationRequestUseCase = new FindNotificationsByParticipationRequestUseCase();
    (findNotificationsByParticipationRequestUseCase as any).notificationRepository = mockNotificationRepository;
  });

  it('should return an notification by user id', async () => {
    mockNotificationRepository.findMany.mockResolvedValue([mockNotification]);

    const result = await findNotificationsByParticipationRequestUseCase.execute(1);

    expect(mockNotificationRepository.findMany).toHaveBeenCalledWith({
      where: { participationRequest : {id: 1 } },
    });
  ...
  it('should return [] if no notification is found', async () => {
    ...
    const result = await findNotificationsByParticipationRequestUseCase.execute(10);
  ...
  it('should throw an error if there is an issue with the repository', async () => {
    ...
    await expect(findNotificationsByParticipationRequestUseCase.execute(1))
      .rejects
